...
public class BinaryHeap {
	...
	public BinaryHeap(int size) {
		//We are adding 1 here so that first cell of the array can be left blank all the time. This is eliminating problem of array starting from index 0.
		arr = new int[size+1];
		this.sizeOfTree = 0;
	...
	public void HeapifyBottomToTop(int index) {
		int parent = index / 2;
		// We are at root of the tree. Hence, no more heapify is required.
		if (index <= 1) {
			return;
		}
	...
	public void HeapifyTopToBottom(int index) {
		...
		if (sizeOfTree < left) { //If there is no child of this node, then nothing to do. Just return.
			...
			if(arr[index] > arr[smallestChild]) { //If Parent is greater than the smallest child, then swap
				int tmp = arr[index];
